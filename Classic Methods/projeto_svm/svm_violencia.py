# -*- coding: utf-8 -*-
"""SVM - Classificação de Violência contra a Mulher"""
...
with open(metricas_path, 'w', encoding='utf-8') as f:
    ...
    for i in range(5):
        ...
        svm_model = SVC(kernel='rbf', random_state=42 + i)
    ...
    f.write("=" * 60 + "\n")
    f.write(f"Acurácia média: {np.mean([accuracy_score(y_teste, SVC(kernel='rbf', random_state=42 + j).fit(X_treino, y_treino).predict(X_teste)) for j in range(5)]):.4f}\n")
    f.write("=" * 60 + "\n")
...
svm_final.fit(X_treino, y_treino)
y_pred_final = svm_final.predict(X_teste)
